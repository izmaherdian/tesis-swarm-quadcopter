...
\section{Sistem Lokalisasi \textit{Indoor} Berbasis \textit{Ultra-Wideband} (UWB)}

Mengingat pengujian navigasi kawanan quadcopter dilakukan di lingkungan dalam ruangan (\textit{indoor}), sistem tidak dapat mengandalkan \textit{Global Positioning System} (GPS) untuk mendapatkan informasi posisi spasial absolut \cite{hoDesignIndoorPositioning2023}. Sebagai gantinya, arsitektur perangkat keras ini memanfaatkan teknologi \textit{Ultra-Wideband} (UWB) sebagai sistem lokalisasi utama. Pada implementasi ini, perangkat keras yang digunakan merujuk pada modul \textit{transceiver} UWB Decawave seri DW1000 (atau variannya seperti DWM1000). Teknologi UWB cip ini dipilih karena memiliki keunggulan berupa ketahanan yang tinggi terhadap interferensi pantulan sinyal (\textit{multipath fading}), konsumsi daya yang efisien, serta mampu memberikan tingkat presisi pelacakan posisi dalam skala sentimeter.

Bentuk fisik dari modul UWB yang diintegrasikan pada sistem wahana dapat dilihat pada Gambar~\ref{fig:uwb_module}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\textwidth]{figures/uwb_module.jpg}
    \caption{Modul \textit{Ultra-Wideband} (UWB) yang digunakan sebagai sistem lokalisasi \textit{indoor} \cite{hoDesignIndoorPositioning2023}}
    \label{fig:uwb_module}
\end{figure}

Sistem lokalisasi UWB yang diimplementasikan terdiri dari dua komponen utama:
\begin{itemize}
    \item \textbf{\textit{Anchor} (Jangkar):} Merupakan sekumpulan modul UWB statis yang dipasang pada posisi tetap dengan koordinat yang telah diketahui secara pasti di sudut-sudut area pengujian. \textit{Anchor} ini berfungsi sebagai patokan kerangka referensi global (\textit{global reference frame}).
    \item \textbf{\textit{Tag} (Label):} Merupakan modul UWB dinamis yang terpasang pada masing-masing quadcopter (terhubung dengan mikrokontroler ESP32). \textit{Tag} ini bergerak mengikuti wahana dan secara aktif berkomunikasi dengan \textit{anchor} di sekitarnya.
\end{itemize}

Proses penentuan jarak absolut antara \textit{tag} dan \textit{anchor} dilakukan menggunakan metode \textit{Two-Way Ranging} (TWR). Metode ini menghitung waktu tempuh sinyal (\textit{Time of Flight} / ToF) saat pesan dikirim dari \textit{tag} ke \textit{anchor} dan kembali lagi ke \textit{tag}. Jarak absolut ($d_i$) antara \textit{tag} dan \textit{anchor} ke-$i$ dirumuskan sebagai berikut:
\begin{equation}
    d_i = c \cdot t_{\text{ToF}, i} = c \cdot \left( \frac{t_{\text{round}} - t_{\text{reply}}}{2} \right)
\end{equation}
di mana $c$ adalah kecepatan rambat cahaya di udara ($\approx 3 \times 10^8$ m/s), $t_{\text{round}}$ adalah total waktu siklus pengiriman dan penerimaan pesan balik, sedangkan $t_{\text{reply}}$ adalah jeda waktu pemrosesan internal pada modul \textit{anchor} sebelum mengirimkan balasan.

Setelah mendapatkan jarak ($d_i$) ke setidaknya empat \textit{anchor} berbeda (untuk lokalisasi 3D), posisi \textit{tag} wahana $(x, y, z)$ dikalkulasi secara \textit{real-time} menggunakan algoritma trilaterasi spasial. Persamaan geometris yang membentuk sistem ini adalah sekumpulan persamaan bola:
\begin{equation}
    (x - x_i)^2 + (y - y_i)^2 + (z - z_i)^2 = d_i^2, \quad \text{untuk } i = 1, 2, \dots, N
\end{equation}
di mana $(x_i, y_i, z_i)$ adalah koordinat tetap dari \textit{anchor} ke-$i$, dan $N \ge 4$ adalah jumlah \textit{anchor} yang terdeteksi. Sistem persamaan non-linear ini kemudian diselesaikan menggunakan metode aproksimasi numerik (seperti \textit{Least Squares}) untuk menghasilkan estimasi posisi 3D kasar dari wahana.

Meskipun UWB mampu memberikan referensi posisi absolut yang sangat baik di lingkungan bebas GPS, keluaran datanya masih mengandung derau pengukuran (\textit{measurement noise}) dan memiliki laju pembaruan (\textit{update rate}) yang relatif lebih lambat dibandingkan dengan dinamika quadcopter. Oleh karena itu, data posisi spasial dari sistem UWB ini tidak dapat langsung diumpankan ke kalang pengontrol tingkat rendah. Data tersebut harus difusikan terlebih dahulu dengan data sensor internal berkecepatan tinggi melalui modul \textit{Extended Kalman Filter} (EKF\label{abbr:ekf}).
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[
        ...
        \node[label, rotate=90] at (-4.35,0.5) {Dinding\\kiri};
        \node[label, rotate=-90] at (4.35,0.5) {Dinding\\kanan};
        ...
        \draw[->, ultra thick, red!70!black] (0, 0.45) -- (0, 1.4) node[above, font=\small\bfseries] {Depan (Arah Gerak)};
        ...
        \node[label, font=\scriptsize] at (-1.5, 0.6) {Sensor\\kiri};
        \node[label, font=\scriptsize] at (1.5, 0.6) {Sensor\\kanan};
...
Pelaksanaan penelitian ini membutuhkan beberapa komponen pendukung untuk merealisasikan sistem navigasi kawanan quadcopter, terutama pada aspek persepsi rintangan, lokalisasi, dokumentasi visual, dan keselamatan pengujian. Perangkat utama yang digunakan meliputi sensor ultrasonik sebagai pendeteksi jarak lateral, kamera sudut lebar untuk dokumentasi dan pengamatan visual, modul UWB sebagai perangkat lokalisasi, serta perlengkapan keselamatan untuk mendukung eksperimen di lingkungan \textit{indoor}.

Pemilihan sensor ultrasonik dilakukan karena kebutuhan penelitian berfokus pada deteksi jarak lateral terhadap rintangan sederhana, sehingga perangkat ini dinilai lebih ringkas dan ekonomis dibandingkan penggunaan kamera kedalaman (\textit{depth camera}). Selain itu, jumlah sensor ultrasonik disesuaikan dengan konfigurasi lima agen quadcopter, yaitu dua sensor pada setiap agen untuk memantau sisi kiri dan kanan wahana.
...
\begin{table}[htbp]
    \centering
    \caption{Rencana Anggaran Biaya (RAB\label{abbr:rab}) Komponen Utama Wahana}
    ...
    \begin{tabularx}{\textwidth}{|c|X|>{\centering\arraybackslash}p{1.9cm}|>{\raggedleft\arraybackslash}p{2.4cm}|>{\raggedleft\arraybackslash}p{2.3cm}|}
        ...
        2 & Camera Wide Angle & 1 & 100.000 & 100.000 \\ \hline
        3 & Modul UWB\label{abbr:uwb} (DWM1000) & 5 & 750.000 & 3.750.000 \\ \hline
        4 & Dus sebagai rintangan & 10 & 30.000 & 300.000 \\ \hline
        5 & Alat \textit{Safety} & 1 & 500.000 & 500.000 \\ \hline
        \multicolumn{4}{|r|}{\textbf{TOTAL HARGA}} & \textbf{4.850.000} \\ \hline
    \end{tabularx}
    \endgroup
\end{table}

\textit{Catatan:} Estimasi biaya pada Tabel~\ref{tab:alat_bahan} hanya mencakup komponen tambahan yang diperlukan untuk penelitian. Komponen utama wahana seperti rangka quadcopter, motor, ESC\label{abbr:esc}, baterai, \textit{flight controller}, dan \textit{companion computer} diasumsikan telah tersedia sebagai bagian dari fasilitas laboratorium atau perangkat penelitian yang telah ada. Modul UWB sebanyak 5 buah dialokasikan sebagai \textit{tag} dinamis pada masing-masing wahana, sedangkan \textit{anchor} statis diasumsikan telah tersedia pada infrastruktur laboratorium pengujian.
