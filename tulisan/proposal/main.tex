...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|c|>{\raggedright\arraybackslash}p{3.6cm}|>{\raggedright\arraybackslash}X|>{\centering\arraybackslash}p{1.6cm}|}
        ...
        14 & Komputer stasiun darat dan penghala Wi-Fi khusus & Laptop Lenovo AMD Ryzen 5 4500U (6 inti), memori 14 GiB, Ubuntu 26.04, dua pengontrol USB 3.1; penghala TP-Link Archer C54 dwipita & tersedia \\ \hline
    \end{tabularx}
    \endgroup
\end{table}
...
Penelitian dibagi ke dalam lima fase (Gambar~\ref{fig:diagram_metodologi}). Tiga titik keputusan menjadi gerbang kelayakan, yaitu evaluasi HITL pada Fase~3, kestabilan wahana tunggal, dan keberhasilan misi kawanan pada Fase~4. Bila gerbang tidak terlewati, pekerjaan kembali ke penanaman logika, bukan ke awal penelitian.
...
\begin{enumerate}
    ...
    Tahap ini dinyatakan lolos bila kriteria Gerbang~2 pada Tabel~\ref{tab:gerbang} terpenuhi.

    \item \textbf{Uji terbang kawanan.} Kamera kedua dipasang dan kedua kamera dikalibrasi ke satu kerangka acuan global. Berdasarkan inventaris (Tabel~\ref{tab:komponen_tersedia}), wahana kelima dirakit dari rangka kosong dan komponen lepas, sehingga pada tahap ini diadakan satu ESC, satu \textit{companion computer} cadangan, propeler, dan empat baterai 6S agar tersedia enam baterai untuk lima wahana. Pengujian kawanan dijalankan dalam dua kondisi. Kondisi kontrol menerbangkan kelima wahana dalam formasi V tanpa rekonfigurasi di koridor terbuka tanpa rintangan, yang memberi nilai acuan $e_{\mathrm{form}}$ dan $\Phi$ pada perangkat keras yang sama. Kondisi uji menerbangkan kawanan dengan ERC aktif melintasi arena (Subbab~\ref{sec:arena}), sehingga lebar ruang $w_e$ yang turun melewati ambang memicu rekonfigurasi, formasi menyusut, beralih mengekor melalui celah $90$~cm, lalu mengembang kembali.

    Formasi kaku tidak ikut diterbangkan menembus celah karena bentangnya $2{,}05$~m sedangkan celah hanya $0{,}90$~m, sehingga tabrakan merupakan kepastian geometris dan bukan hasil pengukuran. Kasus kegagalan tersebut telah tercakup pada simulasi, dan pembanding kuantitatif untuk kondisi uji diambil dari simulasi arena berskala Fase~1.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|p{11.5cm}|c|c|c|c|c|c|c|c|c|}
        ...
        9 & Uji terbang kawanan dengan formasi V acuan di ruang terbuka &  &  &  &  & \cellcolor{gray!70} & \cellcolor{gray!70} &  &  &  \\ \hline
        10 & Uji terbang kawanan dengan formasi adaptif ERC melintasi celah $90$ cm &  &  &  &  &  & \cellcolor{gray!70} & \cellcolor{gray!70} & \cellcolor{gray!70} &  \\ \hline
        11 & Karakterisasi trafik UDP dan latensi komunikasi &  &  &  &  &  &  & \cellcolor{gray!70} & \cellcolor{gray!70} &  \\ \hline
        \multicolumn{11}{|l|}{\cellcolor{gray!10}\textbf{Fase 5: Analisis Data dan Pelaporan}} \\ \hline
        12 & Ekstraksi log pose EKF, riwayat pemicu ERC, dan trafik UDP &  &  &  &  &  &  &  &  & \cellcolor{gray!70} \\ \hline
