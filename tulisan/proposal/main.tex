...
Penelitian ini karenanya memakai firmware ArduPilot racikan yang dibangun melalui ArduPilot Custom Firmware Builder \cite{ardupilotCustomFirmwareBuilder}: fitur navigasi eksternal EKF3 diaktifkan dan fitur yang tidak dipakai dinonaktifkan agar muat dalam 1~MB. Keberhasilan build ini menjadi bagian gerbang kelayakan Fase~2. Bila tidak muat, kalang posisi dan kecepatan dipindahkan ke \textit{companion computer}, dan \textit{flight controller} hanya menjalankan kalang sikap. Daftar fitur yang diaktifkan dan dinonaktifkan dicatat sebagai lampiran agar firmware dapat dibangun ulang.
...
\end{equation}
Vektor masukan $\mathbf{u}_{k-1}$ berasal dari pembacaan sensor internal IMU bawaan \textit{flight controller} (ICM-42688P) pada laju yang jauh lebih tinggi daripada laju kamera, yang terdiri atas akselerasi linear bodi $\mathbf{a}_B = [a_x, a_y, a_z]^T$ dan laju rotasi bodi $\boldsymbol{\omega}_B = [p, q, r]^T$. Sementara itu, vektor pengukuran $\mathbf{z}_k \in \mathbb{R}^4$ diperoleh secara asinkron dari sistem kamera atas berbasis penanda visual AprilTag pada frekuensi $60$ Hz melalui paket MAVLink:
\begin{equation}
    \mathbf{z}_k = 
    \begin{bmatrix}
