...
\end{equation}
Vektor masukan $\mathbf{u}_{k-1}$ berasal dari pembacaan sensor internal IMU bawaan \textit{flight controller} (ICM-42688P) pada laju yang jauh lebih tinggi daripada laju kamera, yang terdiri atas akselerasi linear bodi $\mathbf{a}_B = [a_x, a_y, a_z]^T$ dan laju rotasi bodi $\boldsymbol{\omega}_B = [p, q, r]^T$. Sementara itu, vektor pengukuran $\mathbf{z}_k \in \mathbb{R}^4$ diperoleh secara asinkron dari sistem kamera atas berbasis penanda visual AprilTag hingga $50$ kali per detik melalui paket MAVLink, yaitu
\begin{equation}
    \mathbf{z}_k = 
    \begin{bmatrix}
...
Setiap kali pose absolut dari kamera atas tiba (hingga $50$ kali per detik), koreksi pengukuran dieksekusi untuk mereduksi akumulasi \textit{drift} dari integrasi IMU melalui
\begin{align}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|c|X|>{\centering\arraybackslash}p{1.35cm}|>{\raggedleft\arraybackslash}p{2.1cm}|>{\raggedleft\arraybackslash}p{2.8cm}|}
        ...
        1 & Kamera USB 3.0 ELP 5 MP \textit{global shutter} OG05B10 & 1 unit & 6.600.000 & 6.600.000 \\ \hline
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|c|X|>{\centering\arraybackslash}p{1.35cm}|>{\raggedleft\arraybackslash}p{2.1cm}|>{\raggedleft\arraybackslash}p{2.8cm}|}
        ...
        2 & Kamera USB 3.0 ELP 5 MP \textit{global shutter} OG05B10 kedua & 1 unit & 6.600.000 & 6.600.000 \\ \hline
...
Rencana Anggaran Biaya (RAB\label{abbr:rab}) dirangkum pada Tabel~\ref{tab:rekap_rab}. Harga bersumber dari RAB awal penulis dan survei Tokopedia pada September 2026. Bila terdapat beberapa listing, dipakai harga tertinggi. Harga kamera mengacu pada listing dengan lensa varifokal bawaan, sedangkan harga dengan lensa sudut lebar masih dikonfirmasi ke penjual.
