...
Memuat tinjauan literatur penelitian terdahulu yang relevan (\textit{state of the art}) mengenai kontrol formasi \textit{multi-quadcopter}, penghindaran rintangan, dan lokalisasi \textit{indoor}. Bab ini juga menguraikan landasan teori yang dipakai sebagai dasar perancangan, meliputi pemodelan dinamika wahana, lokalisasi visual berbasis penanda, sensor jarak, penapis Kalman terekstensi, serta kontrol kaskade dan kontrol formasi.
...
Wahana yang digunakan adalah \textit{quadcopter} tipe \textit{cinewhoop} dengan propeler bersaluran (\textit{ducted}), berdimensi luar $25 \times 21$ cm dan tinggi $8$ cm termasuk baterai. Bentuk bersaluran ini dipilih pada penelitian sebelumnya karena menutupi propeler secara penuh, sehingga lebih aman untuk penerbangan di dalam ruangan yang berdekatan dengan dinding dan rintangan. Rincian komponen yang telah tersedia dirangkum pada Tabel~\ref{tab:komponen_tersedia}.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|c|>{\raggedright\arraybackslash}p{3.6cm}|X|>{\centering\arraybackslash}p{1.6cm}|}
        \hline
        \rowcolor{gray!20}
        ...
        2 & \textit{Flight controller} SpeedyBee F405 Mini & Mikrokontroler STM32F405 (\textit{flash} 1 MB), IMU ICM-42688P, barometer DSP-310 & 4 unit \\ \hline
        3 & ESC SpeedyBee BLS 35A Mini & 4-in-1, firmware BLHeli\_S & 4 unit \\ \hline
        4 & \textit{Companion computer} Seeed XIAO ESP32-S3 & Wi-Fi 2,4 GHz dan BLE terintegrasi, antarmuka USB-C & 1 unit \\ \hline
        5 & Baterai LiPo & $4 \times 1200$ mAh, $2 \times 1500$ mAh, $1 \times 2200$ mAh, $2 \times 5000$ mAh & 9 unit \\ \hline
        6 & Propeler dan saluran cadangan & Propeler tiga bilah, cincin saluran & beberapa \\ \hline
        7 & Pencetak tiga dimensi beserta filamen & Untuk mencetak dudukan penanda visual dan sensor & 1 unit \\ \hline
    \end{tabularx}
    \endgroup
\end{table}
...
Satu catatan penting menyangkut modul GNSS yang saat ini masih terpasang pada bagian atas wahana. Modul tersebut tidak digunakan karena seluruh pengujian berlangsung di dalam ruangan, dan pelepasannya justru menguntungkan: selain mengurangi bobot, pelepasan itu membebaskan area datar di tengah bodi yang dibutuhkan untuk pemasangan penanda visual, sebagaimana dibahas pada Subbab~\ref{sec:lokalisasi_rancangan}.

\subsection{Kondisi Ruang Pengujian}
\label{subsec:kondisi_ruang}

Pengujian dilaksanakan di koridor Pusat Teknologi Instrumentasi dan Otomasi (PTIO) ITB. Ruang tersebut bukan laboratorium terbang khusus, melainkan koridor berbentuk huruf L yang juga digunakan sebagai area lalu lintas dan kegiatan pelatihan. Karakteristik ruang yang relevan terhadap perancangan dirangkum pada Tabel~\ref{tab:kondisi_ruang}.
...
Dimensi luar wahana beserta tata letak komponen utamanya diperlihatkan pada Gambar~\ref{fig:sketsa_wahana}. Wahana berukuran $25 \times 21$ cm dengan tinggi $8$ cm termasuk baterai. Empat saluran propeler menempati hampir seluruh permukaan atas, menyisakan jalur datar selebar hanya $4$ cm di sepanjang sumbu tengah bodi. Keterbatasan ruang datar ini berdampak langsung pada perancangan sistem lokalisasi dan dibahas pada Subbab~\ref{sec:lokalisasi_rancangan}.
...
\section{Perancangan Sistem Lokalisasi \textit{Indoor}}
...
Metode ini dipilih karena sistem lokalisasi berbasis kamera eksternal mampu memberikan presisi pose dalam skala sentimeter tanpa akumulasi penyimpangan posisi (\textit{drift}) seiring waktu; galat pose di bawah $3$~cm dan $1^\circ$ tanpa \textit{drift} telah dilaporkan untuk area dalam ruangan seluas sekitar $240$~m$^2$ \cite{bultmannExternalCameraBased2023}. Selain itu, metode ini tidak terpengaruh oleh interferensi gelombang nirkabel (\textit{multipath fading}) yang kerap terjadi pada sensor berbasis sinyal radio di ruangan tertutup. Perlu ditegaskan bahwa presisi yang benar-benar tercapai bukan besaran tetap, melainkan bergantung pada resolusi kamera, ukuran penanda, dan jarak pandang kamera terhadap wahana; akurasi estimasi pose \textit{AprilTag} diketahui menurun seiring bertambahnya jarak dan mengecilnya ukuran penanda \cite{zhangIntegratedFrameworkEnhancing2026}. Ketiga besaran tersebut karenanya ditentukan melalui perhitungan geometri terhadap dimensi area pengujian, bukan diasumsikan.

Sistem lokalisasi kamera atas yang diimplementasikan terdiri dari tiga komponen utama:
\begin{enumerate}
    \item \textbf{Kamera Sudut Lebar (\textit{Wide-Angle Overhead Camera}).} Dua unit kamera dipasang pada posisi statis di langit-langit area pengujian dengan orientasi menghadap tegak lurus ke bawah (\textit{nadir}). Liputan kedua kamera dibuat saling bertumpang tindih dan dikalibrasi ke dalam satu kerangka acuan inersia global (\textit{global reference frame}) sehingga seluruh panjang lintasan uji tercakup. Penggunaan dua unit diperlukan karena tinggi langit-langit ruang pengujian membatasi jarak pandang kamera terhadap wahana, sehingga liputan satu kamera tidak mencukupi panjang lintasan yang diuji.
    \item \textbf{Penanda Visual (\textit{AprilTag} \cite{olsonAprilTagRobustFlexible2011, wangAprilTag2Efficient2016, krogiusFlexibleLayoutsFiducial2019} atau \textit{ArUco} \cite{garridojuradoAutomaticGenerationDetection2014}).} Dipasang secara datar pada bagian atas bodi masing-masing \textit{quadcopter}. Setiap wahana menggunakan pola matriks biner dengan identitas (ID) unik (misalnya ID 0 hingga 4 untuk lima wahana), sehingga sistem persepsi dapat membedakan dan melacak setiap agen secara simultan.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|>{\centering\arraybackslash}X|}
        \hline
        \rowcolor{gray!20}
        \thead{Ketinggian terbang} & \thead{Jarak pandang $d$} & \thead{Sudut pandang horizontal} & \thead{Liputan} \\ \hline
...
\begin{enumerate}
    \item \textbf{Sensor ultrasonik} (HC-SR04). Berbiaya rendah, jangkauan memadai, dan tidak terpengaruh kondisi pencahayaan, namun memiliki berkas lebar dan rentan terhadap pantulan spekular pada sudut datang miring.
    \item \textbf{Sensor jarak laser \textit{time-of-flight}} (VL53L1X). Berkas jauh lebih sempit, lebih toleran terhadap sudut datang miring, dan berkomunikasi melalui antarmuka I\textsuperscript{2}C sehingga hemat pena masukan-keluaran pada \textit{companion computer} yang terbatas. Kelemahannya, jangkauan efektifnya menurun pada pencahayaan terang, sedangkan ruang pengujian menerima cahaya alami melalui kaca \textit{clerestory}.
\end{enumerate}
...
Ketika data posisi absolut dari sistem lokalisasi kamera atas diterima, komponen prediksi apriori dikoreksi menggunakan mekanisme umpan balik pengukuran guna menghasilkan estimasi aposteriori yang optimal:
\begin{align}
...
\subsection{Pengontrol Tingkat Rendah (\textit{Low-Level Controller})}
...
\begin{figure}[htbp]
    \centering
    \makebox[\linewidth][c]{\includegraphics[width=1.2\linewidth]{figures/low_level_controller.png}}
...
\subsection{Perencana Tingkat Tinggi (\textit{High-Level Planner})}
...
Satu prinsip perancangan dipertahankan secara tegas di seluruh arsitektur: \textbf{kamera atas berperan sebagai sistem lokalisasi, sedangkan sensor jarak pada wahana berperan sebagai sistem persepsi}. Pemisahan ini bukan sekadar pembagian teknis. Karena kamera atas mengetahui posisi absolut seluruh agen dan rintangan bersifat statis pada posisi yang diketahui, estimasi lebar ruang $w_e$ secara teknis dapat saja dihitung terpusat di stasiun darat lalu disiarkan ke tiap agen. Namun apabila itu dilakukan, agen tidak lagi mempersepsi lingkungannya sendiri melainkan sekadar membaca peta yang dikirim dari pusat, dan sifat terdesentralisasi yang menjadi landasan ERC menjadi gugur. Dengan mempertahankan pemisahan tersebut, tiap agen tetap memutuskan rekonfigurasi berdasarkan pengukuran yang dilakukannya sendiri.
...
\begin{itemize}
    ...
    \item \textbf{Setup Lokalisasi:} Mengintegrasikan sistem penentuan posisi \textit{indoor} berbasis kamera atas (\textit{AprilTag}) dan mengimplementasikan \textit{Extended Kalman Filter} (EKF) untuk fusi data sensor.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|c|X|>{\centering\arraybackslash}p{1.8cm}|>{\raggedleft\arraybackslash}p{2.3cm}|>{\raggedleft\arraybackslash}p{2.3cm}|}
        ...
        1 & Kamera atas 4K beserta lensa sudut lebar $\pm 109^\circ$ & 1 unit & \textit{survei} & \textit{survei} \\ \hline
        2 & Kabel USB ekstensi 10 m dan dudukan langit-langit & 1 set & 150.000 & 150.000 \\ \hline
        3 & Sensor ultrasonik HC-SR04 (kandidat A) & 2 unit & 20.000 & 40.000 \\ \hline
        4 & Sensor jarak laser VL53L1X (kandidat B) & 2 unit & \textit{survei} & \textit{survei} \\ \hline
        5 & Penanda \textit{AprilTag}, akrilik, dan tiang cetak tiga dimensi & 1 set & 20.000 & 20.000 \\ \hline
        \multicolumn{4}{|r|}{\textbf{SUBTOTAL TAHAP 1}} & \textbf{210.000 + survei} \\ \hline
    \end{tabularx}
    \endgroup
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|c|X|>{\centering\arraybackslash}p{1.8cm}|>{\raggedleft\arraybackslash}p{2.3cm}|>{\raggedleft\arraybackslash}p{2.3cm}|}
        ...
        \rowcolor{gray!20}
        \thead{No} & \thead{Komponen} & \thead{Kuantitas} & \thead{Harga Satuan\\(Rp)} & \thead{Harga Total\\(Rp)} \\ \hline
        1 & Kamera atas 4K kedua beserta lensa dan dudukan & 1 unit & \textit{survei} & \textit{survei} \\ \hline
        2 & Sensor jarak terpilih hasil Tahap 1 & 8 unit & \textit{survei} & \textit{survei} \\ \hline
        3 & Penanda \textit{AprilTag}, akrilik, dan tiang cetak tiga dimensi & 4 set & 20.000 & 80.000 \\ \hline
        \multicolumn{4}{|r|}{\textbf{SUBTOTAL TAHAP 2}} & \textbf{80.000 + survei} \\ \hline
    \end{tabularx}
    \endgroup
\end{table}
...
Tahap ketiga menyiapkan arena pengujian beserta perlengkapan keselamatannya. Penempatannya di akhir bukan karena kurang penting, melainkan karena perlengkapan ini baru diperlukan saat pengujian rekonfigurasi formasi dijalankan secara penuh, sementara Tahap 1 dan Tahap 2 masih berlangsung di ruang terbuka tanpa rintangan. Rincian anggarannya diberikan pada Tabel~\ref{tab:rab_tahap3}.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|c|X|>{\centering\arraybackslash}p{1.8cm}|>{\raggedleft\arraybackslash}p{2.3cm}|>{\raggedleft\arraybackslash}p{2.3cm}|}
        ...
        \rowcolor{gray!20}
        \thead{No} & \thead{Komponen} & \thead{Kuantitas} & \thead{Harga Satuan\\(Rp)} & \thead{Harga Total\\(Rp)} \\ \hline
        1 & Dus kardus sebagai rintangan & 10 unit & 30.000 & 300.000 \\ \hline
        2 & Perlengkapan keselamatan & 1 set & 500.000 & 500.000 \\ \hline
...
Komponen yang harganya telah pasti berjumlah Rp1.140.000, terdiri atas Rp210.000 pada Tahap 1, Rp80.000 pada Tahap 2, dan Rp850.000 pada Tahap 3. Beberapa komponen masih menunggu survei harga pasar, yaitu kamera atas 4K beserta lensanya pada Tahap 1 dan Tahap 2 serta sensor jarak laser sebagai kandidat pembanding. Angka-angka tersebut sengaja tidak diperkirakan tanpa dasar dan akan dilengkapi setelah survei dilakukan.

Estimasi biaya ini bersifat awal dan dapat berubah menyesuaikan ketersediaan komponen serta harga pasar. Namun demikian, penyusunan bertahap memberikan gambaran kebutuhan minimum yang harus dipenuhi pada setiap titik pencapaian, sekaligus membatasi risiko finansial apabila salah satu asumsi perancangan tidak terbukti di lapangan.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|p{11.5cm}|c|c|c|c|c|c|c|c|c|}
        ...
        5 & Setup Lokalisasi: Integrasi Kamera Atas \& Extended Kalman Filter (EKF) & & \cellcolor{gray!70} & \cellcolor{gray!70} & & & & & & \\ \hline
